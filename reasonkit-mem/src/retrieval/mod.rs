...
impl HybridRetriever {
    ...
    ///
    /// Note: For pure dense search, doc_id and text are retrieved from BM25 index
    /// if available. If chunk is not in BM25 (e.g., embedding-only documents),
    /// doc_id will be Uuid::nil() and text will be empty. Use `search_hybrid`
    /// for complete metadata when possible.
    pub async fn search_dense(&self, query: &str, top_k: usize) -> Result<Vec<HybridResult>> {
        ...
        for (chunk_id, score) in dense_results {
            // Attempt to get chunk metadata from BM25 index
            let (doc_id, text) = self
                .index
                .get_chunk_by_id(&chunk_id)
                .map(|chunk_info| (chunk_info.doc_id, chunk_info.text.clone()))
                .unwrap_or_else(|| {
                    // Chunk not in BM25 index (embedding-only document)
                    (Uuid::nil(), String::new())
                });
    ...
    pub async fn search_hybrid(
        ...
        // Fuse results using default RRF strategy
